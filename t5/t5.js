...
// your code here
restaurants.forEach(
  r => (r.location.coordinates = r.location.coordinates.reverse())
);

const map = L.map('map').setView([...restaurants[0].location.coordinates], 10);
...
for (const restaurant of restaurants) {
  const marker = L.marker([...restaurant.location.coordinates]).addTo(map);
  marker.bindPopup(`<p><h3>${restaurant.name}</h3>${restaurant.address}</p>`);
}
